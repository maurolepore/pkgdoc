...
pick1 |> knitr::kable(col.names = NULL)
```
...
pick2 |> DT::datatable(escape = FALSE)
```
...
```{r}
by_concept <- filter(pick2, concept == "Datasets available by data()")
...
```{r}
by_package <- filter(pick2, package == "tools")
